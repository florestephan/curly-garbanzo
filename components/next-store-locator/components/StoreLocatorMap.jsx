...
const StoreLocatorMap = React.memo(({
        ...
        loader.load().then(async () => {
            ...
            if (locations) {
                const locationsData = locations.map(loc => ({
                    ...loc,
                    position: new google.maps.LatLng(parseFloat(loc.location.lat), parseFloat(loc.location.lng))
                }));
    ...
    const addMarkers = (storeData) => {
        ...
        markerCluster.current = new MarkerClusterer(map.current, [], {
            styles: [{
                url: iconPaths.markerIcon ?? defaultClusterIcon,
                // icon: iconPaths.markerIcon ?? defaultMarkerIcon,
                width: 40,
                height: 34,
                textColor: '#484848',
    ...
    return (
        <div className="col-span-6 lg:col-span-4">
            <div ref={mapRef}
                 style={{width: '100%', height: '80vh', backgroundColor: 'background-color: rgb(229, 227, 223)'}}
                 className="h-[40vh] lg:h-[80vh]"/>
            <button onClick={getUserLocation}
                    className="bg-white p-2 absolute top-[5px] left-[10px] lg:top-[10px] lg:left-[30px]">
